...
const FF_DEFAULTS = {
 ...
 sendWhole: true,        // ส่งทั้งไฟล์เข้าไปเลย ไม่ต้องให้บอทนั่งขอทีละท่อน
 wholeMaxKB: 220,        // ไฟล์ใหญ่กว่านี้ถึงจะถอยไปใช้โหมดส่งเป็นท่อน
 autoRepair: true,       // แพตช์ทำโครงสร้างพัง ให้ส่งกลับไปบอกบอทให้แก้ ไม่ใช่ทิ้งเงียบ
 repairMax: 3,           // ให้แก้ตัวเองได้กี่รอบ
};
...
function ffBalance(text) {
 ...
 const st = [];
 let i = 0, line = 1;
 let prev = '';                      // ตัวอักษรที่ไม่ใช่ช่องว่างตัวก่อนหน้า ใช้แยก regex กับหาร
 while (i < src.length) {
  ...
  // template literal ต้องเดินผ่าน ${...} ให้ถูก เพราะข้างในเป็นโค้ดจริง
  if (c === '`') {
   i++;
   let depth = 0;
   while (i < src.length) {
  ...
  /* ★ regex literal — ต้นเหตุที่ตัวตรวจรุ่นก่อนเตือนผิด
     ปีกกาใน /\}/g หรือ [^{] ถูกนับเป็นของจริงจนบอกว่าไฟล์พัง ทั้งที่ไฟล์ถูกต้อง
     แยกจากเครื่องหมายหารด้วยตัวอักษรก่อนหน้า ถ้าเป็นค่าอยู่แล้วแสดงว่าเป็นการหาร */
  if (c === '/') {
   const valueBefore = /[\w$)\]]/.test(prev);
   if (!valueBefore) {
    i++;
    let inClass = false;
    while (i < src.length) {
     if (src[i] === '\\') { i += 2; continue; }
     if (src[i] === '\n') { line++; break; }      // regex ไม่ข้ามบรรทัด ถ้าเจอแปลว่าเดาผิด
  ...
  if (pairs[c]) { st.push({ c, line }); prev = c; i++; continue; }
  if (close[c]) {
   const top = st.pop();
...
function ffBuildBlock() {
 ...
 // ★ โหมดส่งทั้งไฟล์ บอทเห็นทุกบรรทัดพร้อมเลขบรรทัด แก้ได้ทันทีในรอบเดียว
 const kb = f.text.length / 1024;
 if (s.sendWhole && kb <= (s.wholeMaxKB || 220)) {
  const numbered = ffLines(f.text).map((l, i) => `${i + 1}| ${l}`).join('\n');
  parts.push(`เนื้อไฟล์ทั้งหมดของ ${f.name} (ตัวเลขหน้าขีดคือเลขบรรทัด ห้ามคัดลอกเลขบรรทัดลงใน FIND)\n${numbered}`);
  return parts.join('\n\n');
 }
 if (s.sendWhole) parts.push(`ไฟล์นี้ใหญ่ ${Math.round(kb)} KB เกินกว่าจะส่งทั้งไฟล์ จึงส่งเป็นท่อนแทน`);
 if (!ffSlices.length) {
...
function ffApply(results) {
 ...
 touched.forEach((before, id) => {
  ...
  if (!bal.ok) {
   /* ★ ไม่ทิ้งงานเฉย ๆ
      เก็บสภาพที่พังไว้ดูได้ ย้อนไฟล์กลับให้ปลอดภัย แล้วส่งข้อผิดพลาดกลับไปให้บอทแก้เอง
      เหมือนคนแก้โค้ดจริงที่เจอ error แล้วแก้ต่อ ไม่ใช่ล้มเลิก */
   const broken = f.text;
   f.text = before;
   f.edits = (f.edits || []).filter(e => before.includes(e.after));
   n = 0;
   ffPush('bad', `${f.name} โครงสร้างพังหลังแพตช์`, bal.why);
   ffRepair(f, bal, broken);
  } else {
...
/** ★ ส่งข้อผิดพลาดกลับไปให้บอทแก้เอง พร้อมโค้ดรอบจุดที่พัง */
let ffRepairCount = 0;
function ffRepair(f, bal, brokenText) {
 const s = ffSettings();
 if (!s.autoRepair) {
  ffToast('ย้อนคืนแล้ว', `${f.name}: ${bal.why}`, 'error');
  return;
 }
 if (ffRepairCount >= (s.repairMax || 3)) {
  ffRepairCount = 0;
  ffToast('แก้ไม่หายสักที', `${bal.why} — ลองบอกบอทตรง ๆ ว่าให้แก้ยังไง`, 'error');
  return;
 }
 ffRepairCount++;
 // ดึงโค้ดรอบบรรทัดที่พังจากสภาพที่พัง เพื่อให้บอทเห็นว่าตัวเองทำอะไรผิด
 const m = String(bal.why).match(/บรรทัด (\d+)/);
 const ln = m ? +m[1] : 1;
 const bl = ffLines(brokenText);
 const a = Math.max(1, ln - 14), b = Math.min(bl.length, ln + 14);
 const ctx = bl.slice(a - 1, b).map((l, i) => `${a + i}| ${l}`).join('\n');
 ffPush('warn', `ส่งกลับให้บอทแก้ รอบที่ ${ffRepairCount}`, bal.why);
 ffToast('แพตช์ทำโครงสร้างพัง', `${bal.why} — ส่งกลับให้บอทแก้เองแล้ว`, 'warning');
 const msg = [
  `แพตช์ที่เพิ่งส่งมาทำให้ ${f.name} โครงสร้างพัง`,
  `ข้อผิดพลาด: ${bal.why}`,
  `ระบบย้อนไฟล์กลับเป็นสภาพก่อนแก้ให้แล้ว เพื่อไม่ให้ไฟล์เสีย`,
  `นี่คือโค้ดรอบจุดที่พัง จากสภาพหลังแพตช์ของคุณ`,
  '```',
  ctx,
  '```',
  `ตรวจว่าปีกกาหรือวงเล็บขาดหรือเกินตรงไหน แล้วส่ง ff-patch ชุดใหม่ที่ถูกต้องมาแทน`,
  `อย่าส่งชุดเดิมซ้ำ`,
 ].join('\n');
 try {
  const ta = document.getElementById('send_textarea');
  const btn = document.getElementById('send_but');
  if (!ta || !btn || ta.value.trim()) { ffToast('File Forge', 'พิมพ์ส่งเองได้เลย บอทจะเห็นข้อผิดพลาด', 'info'); return; }
  ta.value = msg;
  ta.dispatchEvent(new Event('input', { bubbles: true }));
  setTimeout(() => btn.click(), 150);
 } catch (e) { console.error(`[${FF_NAME}] repair`, e); }
}
 ...
 <div class="fileforge-settings">
  <div class="inline-drawer">
   ...
   <div class="inline-drawer-content">
    ...
    ${sw('ff-s-whole', 'ส่งทั้งไฟล์ให้บอทเลย', 'ไม่ต้องนั่งเปิดทีละท่อน บรีฟอย่างเดียว · ไฟล์ใหญ่มากจะถอยไปส่งเป็นท่อนเอง', s.sendWhole)}
    ${sw('ff-s-repair', 'แพตช์พังแล้วให้บอทแก้เอง', 'ส่งข้อผิดพลาดกับโค้ดรอบจุดนั้นกลับไปให้มันแก้ ไม่ทิ้งงาน', s.autoRepair)}
    ${sw('ff-s-botsearch', 'ให้บอทค้นหาและเปิดดูโค้ดเอง', 'ใช้ตอนไฟล์ใหญ่เกินส่งทั้งไฟล์', s.botSearch)}
...
function ffBind() {
 ...
 chk('ff-s-whole', 'sendWhole');
 chk('ff-s-repair', 'autoRepair');
